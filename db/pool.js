...
class StorageHandler {
    ...
    async newUser(body) {
        ...
        try {
            ...
            results = await client.query(
                'INSERT INTO users(Email, Password) VALUES($1, $2) RETURNING id', [input.user.username, input.user.password]
            );
    ...
    async loadUser(body) {
        ...
        try {
            await client.connect();
            results = await client.query(
                'SELECT * FROM users WHERE email = $1 AND password = $2', [input.user.username, input.user.password]
            );
            results = results.rows[0].id;
    ...
    async deleteUser(body) {
        ...
        try {
            await client.connect();
            results = await client.query(
                'DELETE * FROM users WHERE id = $1 AND password = $2', [input.id, input.password]
            );
            userid = results.rows[0].id;
            console.log(`user with id: ${userid} deleted`);

            results = await client.query(
                'DELETE * OUTPUT DELETED.presentationid FROM presentations WHERE userid = $1 ', [userid]
            );
            console.log(results);
    ...
    async loadPres(body) {
        ...
        try {
            ...
            let results = await client.query(
                'SELECT COUNT(userid) FROM presentations WHERE userid = $1', [input.user.id]
            );

            let rowcount = results.rows[0].count;

            results = await client.query(
                'SELECT * FROM presentations WHERE userid = $1', [input.user.id]
            );
    ...
    async createPres(inp) {
        ...
        try {
            await client.connect();
            results = await client.query(
                'INSERT INTO presentations(userid, title, data)VALUES($1,$2, $3) RETURNING presentationid', [input.userid, input.title, input.slides] //input 1=userid, input 2 = data
            );
    ...
    async updatePres(inp) {
        ...
        try {
            ...
            result = await client.query(
                'UPDATE presentations SET data =$1 WHERE presentationid = $2 RETURNING presentationid', [input[1], input[0]]
            );
