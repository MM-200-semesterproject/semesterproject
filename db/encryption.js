...
exports.hashCode = function (userData) {
  ...
  let returnObject = {
    user: { username: createHash(username), password: createHash(password) },
  };
...
exports.singleHash = function (inpString) {
  let string = inpString.user.password;
  let inpId = inpString.user.id;
  let hash = 0;
  for (let i = 0; i < string.length; i++) {
    let charOfString = string.charCodeAt(i);
    hash = (hash << 5) - hash + charOfString; // <<5 shifts bits in binary 5 times left
  ...
  let obj = { id: inpId, password: hash };
  return obj;
};
